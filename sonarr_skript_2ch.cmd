...
if defined sonarr_eventtype (
	...
	if "%sonarr_eventtype%" == "Download" (
		...
		ffmpeg -i "%sonarr_episodefile_path%.audcon" -map 0:v -map 0:a -map 0:s? -c:v copy -c:s copy -c:a ac3 -b:a 640k -ac 2 -metadata:s:a title="Dolby Digital 2.0" "%sonarr_episodefile_path%"
		echo. >> sonarr_skript.log
