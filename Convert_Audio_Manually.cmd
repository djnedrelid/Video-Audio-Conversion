...
if "%keeporigtrack%"=="no" (
	if "%convertinstead%"=="yes" (
		...
		:: Higher Voices.
		if "%convertaudioto%"=="2" (
			set cmd=-i %sti%.audcon -map 0:v -map 0:s? -map 0:a:0 -c:v copy -c:s copy -c:a:0 ac3 -b:a:0 640k -af "pan=stereo|c0=1.0*FL+1.0*SL+1.0*BL+%highervoiceval%*FC+%LFEstring%|c1=1.0*FR+1.0*SR+1.0*BR+%highervoiceval%*FC+%LFEstring%,volume=-6dB" -metadata:s:a:0 language=%lang% -metadata:s:a:0 title="DD 2.0 HigherVoice" %sti%
...
::
set complex_filter="[0:a:0]asplit=2[out1][out2];[out1]pan=5.1|c0=FL|c1=FR|c2=FC|c3=LFE|c4=SL+BL|c5=SR+BR,volume=-6dB[a1];[out2]pan=stereo|c0=1.0*FL+1.0*SL+1.0*BL+%highervoiceval%*FC+%LFEstring%|c1=1.0*FR+1.0*SR+1.0*BR+%highervoiceval%*FC+%LFEstring%,volume=-6dB[a2]"
