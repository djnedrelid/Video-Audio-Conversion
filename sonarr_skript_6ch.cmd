...
if defined sonarr_eventtype (
	...
	if "%sonarr_eventtype%" == "Download" (
		...
		ffmpeg -i "%sonarr_episodefile_path%.audcon" -map 0:v -map 0:a:0 -map 0:s? -c:v copy -c:s copy -c:a:0 eac3 -b:a 1536k -ac 6 -metadata:s:a:0 title="Dolby Digital+ 5.1" "%sonarr_episodefile_path%"
		echo. >> sonarr_skript.log
