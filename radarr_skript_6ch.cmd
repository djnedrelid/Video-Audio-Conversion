...
if defined radarr_eventtype (
	...
	if "%radarr_eventtype%" == "Download" (
		...
		ffmpeg -i "%radarr_moviefile_path%.audcon" -map 0:v -map 0:a:0 -map 0:s? -c:v copy -c:s copy -c:a:0 eac3 -b:a 1536k -ac 6 -metadata:s:a:0 title="Dolby Digital+ 5.1" "%radarr_moviefile_path%"
		echo. >> radarr_skript.log
